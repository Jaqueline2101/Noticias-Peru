...
response = supabase.table('noticias').select('categoria_principal').execute()

categorias = set()
for item in response.data:
    if item['categoria_principal']:
        categorias.add(item['categoria_principal'])

print("CATEGORÍAS EN DB:")
for c in sorted(list(categorias)):
    print(f"'{c}'")
